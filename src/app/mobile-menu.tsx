...
export function MobileMenu({ brand, view, catalogue, tasks, activity }: { brand: Brand; view: Section; catalogue: CatalogueGroup; tasks: TaskView; activity?: ActivityFilter }) {
  ...
  const link = (nextBrand: Brand, nextView: Section, options: { catalogue?: CatalogueGroup; tasks?: TaskView } = {}) => {
    const group = options.catalogue ?? catalogue;
    const taskView = options.tasks ?? tasks;
    return `/?brand=${nextBrand}&view=${nextView}${nextView === "catalogue" ? `&catalogue=${group}` : ""}${nextView === "tasks" ? `&tasks=${taskView}` : ""}${nextView === "updates" && activity ? `&activity=${activity}` : ""}`;
  };
