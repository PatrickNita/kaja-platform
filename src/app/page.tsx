...
function EntryCard({ brand, type, id, title, body, author, comments, reactions, memberSlug, media, detailsMedia, modal = false, children }: { brand: Brand; type: EntryEntity; id: number; title: string; body?: string; author: string; comments?: CommentRow[]; reactions?: ReactionRow[]; memberSlug: string; media?: ReactNode; detailsMedia?: ReactNode; modal?: boolean; children?: ReactNode }) {
  ...
  const last = own.at(-1);
  const compactPreview = <>{media}<div className="entry-card-head"><h3 className="item-title">{title}</h3><span className="entry-author">{author}</span></div>{body && <p className="entry-preview">{preview(body)}</p>}{commentType && <div className="comment-preview">Comentarii ({own.length}){last && <> · ({last.author} – {date(last.comment.createdAt)})</>}</div>}</>;
  return <><article className="item card entry-card-placeholder" aria-hidden="true">{compactPreview}</article><article className="item card entry-card-surface"><details id={`entry-${type}-${id}`} className="entry-card"><summary>{compactPreview}</summary>{modal && <div className="entry-modal-header"><div className="entry-modal-title-group"><h3 className="item-title">{title}</h3><span className="entry-author">{author}</span></div><button type="button" className="entry-modal-close" data-entry-close aria-label={`Închide ${title}`}>×</button></div>}<div className="entry-card-content">{detailsMedia}{body && <p>{body}</p>}{commentType && <Comments brand={brand} entityType={commentType} entityId={id} comments={comments || []} reactions={reactions || []} memberSlug={memberSlug} />}{children}</div></details></article></>;
}
