...
function WorkspaceSection({ section, title, items }: { section: string; title: string; items: WorkspaceItemRow[] }) {
  ...
  return <section className="panel"><div className="panel-head"><h2>{title}</h2><span className="count">{items.length} active</span></div><form action={createWorkspaceItem} className="form"><input type="hidden" name="section" value={section} /><input name="title" required maxLength={160} placeholder={`${singular} title`} /><textarea name="body" required maxLength={4000} placeholder={`Describe this ${singular.toLowerCase()}.`} /><button className="button">Add {singular}</button></form><div className="items">{items.length === 0 ? <p className="empty">No {title.toLowerCase()} yet. Add the first one.</p> : items.map(({ item, author }) => <article key={item.id} className="item"><h3 className="item-title">{item.title}</h3><p>{item.body}</p><div className="meta"><span>Created by {author}</span><span>·</span><span>{date(item.updatedAt)}</span></div><details><summary>Edit {singular.toLowerCase()}</summary><form action={updateWorkspaceItem} className="form edit-form"><input type="hidden" name="id" value={item.id} /><input type="hidden" name="section" value={section} /><input name="title" defaultValue={item.title} required maxLength={160} /><textarea name="body" defaultValue={item.body} required maxLength={4000} /><button className="button">Save changes</button></form><form action={deleteWorkspaceItem} className="edit-form"><input type="hidden" name="id" value={item.id} /><input type="hidden" name="section" value={section} /><input type="hidden" name="title" value={item.title} /><button className="button ghost danger">Delete {singular.toLowerCase()}</button></form></details></article>)}</div></section>;
}

function UpdatesSection({ data }: { data: NonNullable<Awaited<ReturnType<typeof workspaceData>>> }) {
...
function TasksSection({ data }: { data: NonNullable<Awaited<ReturnType<typeof workspaceData>>> }) {
...
export default async function Home({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  const content = view === "updates" ? <UpdatesSection data={data} /> : view === "tasks" ? <TasksSection data={data} /> : <WorkspaceSection section={view} title={sections.find((section) => section.key === view)!.label} items={sectionItems} />;
